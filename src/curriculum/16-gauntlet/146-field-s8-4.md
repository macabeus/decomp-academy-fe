---
id: gauntlet-field-s8-4
title: Read a s8 field at offset 4
difficulty: 3
concepts:
  - structs
  - loads
...
# Load a `s8` at byte offset 4

A typed field access compiles to a single load with the field's displacement.
A `s8` field reads with **`lbz`** at displacement 4. The *type* picks the load opcode; the *offset* picks the displacement.
...
```c
typedef struct { u8 _pad[4]; s8 field; } S;
```
